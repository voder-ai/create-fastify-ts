...
- [ ] **Server Starts Successfully**: Running `npm run dev` starts the development server without errors
- [ ] **Port Auto-Discovery**: Server starts on default port (3000) or automatically finds next available port if 3000 is in use
- [ ] **Strict Port Mode**: When PORT environment variable is set, server fails with clear error if that specific port is unavailable
- [ ] **Health Check Accessible**: Server responds to GET /health with 200 OK status
- [ ] **Automatic Restart**: Changing TypeScript source files automatically triggers server restart
- [ ] **Fast Restart Time**: Server restarts in under 2 seconds after file changes
- [ ] **Clear Console Output**: Server logs show clear startup messages including listening URL and port (especially important when auto-discovering port)
- [ ] **Error Messages**: TypeScript compilation errors are displayed clearly in console with file/line references
- [ ] **Graceful Shutdown**: Ctrl+C cleanly stops the development server without hanging or errors
...
- [ ] npm run dev starts server successfully in initialized project
- [ ] All acceptance criteria met
- [ ] Server automatically restarts on TypeScript file changes
- [ ] Clear console output with listening URL and port
- [ ] Error messages are helpful and actionable
- [ ] Documentation reflects the dev server workflow
- [ ] Story map updated with story reference
- [ ] Ready for stories 005.0, 008.0, 009.0
